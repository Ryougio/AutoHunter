...
class ExecutorProxyTests(unittest.TestCase):
    ...
    def test_empty_403_does_not_rotate(self) -> None:
        ps.apply_config(_pool(_px(1), _px(2)))
        self.ex._proxy_id = "px-1"
        self.ex._proxy_url = "http://10.0.0.1:8080"
        response = _fake_response()
        ...
        response.iter_bytes = lambda: iter([b""])
        with patch.object(ToolExecutor, "_get_http_client", return_value=FakeClient([response])):
            result = self.ex.http_request("http://target.example/x")
        self.assertTrue(result["ok"])
        self.assertNotIn("waf_blocked", result)
        self.assertEqual(self.ex._proxy_id, "px-1")

    def test_nginx_403_without_block_page_does_not_rotate(self) -> None:
        ps.apply_config(_pool(_px(1), _px(2)))
        self.ex._proxy_id = "px-1"
        self.ex._proxy_url = "http://10.0.0.1:8080"
        response = _fake_response()
        response.status_code = 403
        response.headers = {"server": "nginx", "content-type": "text/html"}
        response.iter_bytes = lambda: iter([b"<html>403 Forbidden</html>"])
        with patch.object(ToolExecutor, "_get_http_client", return_value=FakeClient([response])):
            result = self.ex.http_request("http://target.example/x")
        self.assertTrue(result["ok"])
        self.assertNotIn("proxy_rotated", result)
    ...
    def test_post_waf_rotates_without_replay(self) -> None:
        ps.apply_config(_pool(_px(1), _px(2)))
        self.ex._proxy_id = "px-1"
        self.ex._proxy_url = "http://10.0.0.1:8080"
        blocked = _fake_response()
        blocked.status_code = 403
        blocked.headers = {"server": "cloudflare", "cf-ray": "abc"}
        blocked.iter_bytes = lambda: iter([b"Just a moment... Cloudflare"])
        client = FakeClient([blocked])
        with patch.object(ps._RANDOM, "choice", side_effect=lambda items: items[-1]), \
                patch.object(ToolExecutor, "_get_http_client", return_value=client):
            result = self.ex.http_request("http://target.example/login", method="POST", data="a=1")
        self.assertTrue(result["ok"], result)
        self.assertEqual(result["status_code"], 403)
        self.assertTrue(result["waf_blocked"])
        self.assertFalse(result["proxy_replayed"])
        self.assertNotIn("proxy_rotated", result)
        self.assertEqual(self.ex._proxy_id, "px-2")
        self.assertEqual(len(client._behavior), 0)

    def test_read_timeout_does_not_kill_proxy(self) -> None:
        ps.apply_config(_pool(_px(1), _px(2)))
        bad = FakeClient([httpx.ReadTimeout("slow target")])
        with patch.object(ToolExecutor, "_get_http_client", return_value=bad):
            result = self.ex.http_request("http://target.example/x")
        self.assertFalse(result["ok"])
        self.assertEqual(self.ex._proxy_id, "px-1")
        self.assertNotIn("px-1", self.ex._used_proxy_ids)
        health = ps._health.get("px-1") or {}
        self.assertEqual(int(health.get("fails") or 0), 0)
    ...
    def test_shell_proxy_env_injection(self) -> None:
        ...
        proc = Mock()
        proc.stdout = Mock()
        proc.stdout.read1 = Mock(return_value=b"")
